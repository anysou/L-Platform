        ...
        public void run() {
            ...
            try {
                ...
                if (code == 200) {
                    ...
                    //因为分享的图片都很小，这里以250*250为上限值,只加载图片0~250区域的像素
                    ...
                    mHandler.obtainMessage(LoadHandler.HANDLER_ERROR, "SingleImageLoader：图片加载出错").sendToTarget();
                }
            } catch (IOException e) {
                //将数据返回
                mHandler.obtainMessage(LoadHandler.HANDLER_ERROR, "SingleImageLoader：图片加载网络连接错误").sendToTarget();
            } finally {
                if (conn != null)
                    conn.disconnect();
            }
